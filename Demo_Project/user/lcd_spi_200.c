...
void LCD_WriteCommand(uint8_t lcd_command)
{
	...
	//HAL_SPI_Transmit_DMA(&LCD_SPI, &lcd_command, 1);
}
...
void SPI_LCD_Init(void)
{
	...
	LCD_WriteData_8bit(0x40); // 配置成 从下到上、从右到左，RGB像素格式
...
void LCD_CopyBuffer(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t *DataBuff)
{
    ...
    hspi5.Instance->CFG1 &= 0xFFFFFFE0;
    hspi5.Instance->CFG1 |= SPI_DATASIZE_16BIT;

	LCD_SPI_TransmitBuffer(&LCD_SPI, DataBuff, width * height);

	//HAL_SPI_Transmit(&hspi5, (uint8_t *)DataBuff, width * height, 1000) ;

	// 改回8位数据宽度，因为指令和部分数据都是按照8位传输的
	//LCD_SPI.Init.DataSize = SPI_DATASIZE_8BIT; //	8位数据宽度
	//HAL_SPI_Init(&LCD_SPI);
    hspi5.Instance->CFG1 &= 0xFFFFFFE0;
    hspi5.Instance->CFG1 |= SPI_DATASIZE_8BIT;
}
